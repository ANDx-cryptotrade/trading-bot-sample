...
class PaperBroker:
    ...
    # set by the engine in paper mode: symbol -> live spread fraction.
    # Market orders cross half the spread each way; a demo that fills at
    # mid quietly flatters every trade by the venue's whole spread.
    spread_fn = None

    def _half_spread(self, symbol: str) -> float:
        if not self.spread_fn:
            return 0.0
        try:
            return max(0.0, float(self.spread_fn(symbol))) / 2
        except Exception:
            return 0.0

    def open(self, symbol, side, qty, price, stop, take_profit, strategy) -> Position:
        fill = price * (1 + (self.slip_for(qty * price) + self._half_spread(symbol)) * side)
    ...
    def close(self, symbol, price) -> dict | None:
        ...
        if not pos:
            return None
        fill = price * (1 - (PAPER_SLIPPAGE + self._half_spread(symbol)) * pos.side)
...
class AndxBroker(PaperBroker):
    ...
    def __init__(self, creds: dict):
        ...
        self.client = andx.AndxClient(
            ...
            session_jwt=creds.get("session_jwt", ""),
        )
        self._sync_balance()
