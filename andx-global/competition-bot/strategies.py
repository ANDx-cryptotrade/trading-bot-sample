...
class AutoRegime(Strategy):
    ...
    def __init__(self, params=None):
        ...
        # The leg that opened the current position. Whichever logic pulled
        # the trigger manages the trade to its end: handing a trend rider to
        # the mean-reversion exit (or vice versa) on a mid-trade ADX wobble
        # was measured to kill half of all winning entries.
        self._open_leg: Strategy | None = None

    @property
    def trailing(self):  # type: ignore[override]
        if self._open_leg is not None:
            return self._open_leg is self.trend
    ...
    def signal(self, df, current):
        ...
        if current != 0:
            # A position is open: its opening leg decides when it ends.
            # (After a restart the leg is unknown — fall back to the leg
            # the current regime favors.)
            leg = self._open_leg or (
                self.trend if self._last_regime == "trend" else self.meanrev)
            self._open_leg = leg
            desired = leg.signal(df, current)
            if desired != current:
                self._open_leg = None
            return desired
        self._open_leg = None
        ...
        desired = active.signal(df, current)
        if desired != 0:
            self._open_leg = active
        return desired
