...
# ANDX_BOT_PORT lets the gateway run one instance per user on its own port;
# the default keeps the single-user desktop setup on 8300.
PORT = int(os.environ.get("ANDX_BOT_PORT", "8300"))
...
def order_action():
    body = request.get_json(silent=True) or {}
    if str(body.get("action", "")).lower() != "cancel":
        return jsonify({"ok": False, "error": "action must be cancel"}), 400
    result = ENGINE.cancel_pending_order(body.get("id"))
    ok = "error" not in result
    return jsonify({"ok": ok, **result}), (200 if ok else 400)
...
@app.get("/api/backtest_record")
def backtest_record():
    """Labeled simulated record — separate from live/paper trading data."""
    return jsonify(store.backtest_record())
...
if __name__ == "__main__":
    print(f"\n  ANDX Trading Bot — dashboard: http://127.0.0.1:{PORT}\n")
