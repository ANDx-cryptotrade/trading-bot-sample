...
from strategies import make_strategy, atr, adx, htf_bias
...
class BotEngine:
    def __init__(self):
        ...
        # symbol -> (consecutive contra-signal count, ts of the bar counted):
        # trend positions only signal-exit after the contra reading holds on
        # two different closed candles, so one bar's wobble can't churn fees
        self._contra: dict[str, tuple[int, float]] = {}
        # symbol -> ts until which new entries are blocked after ANY exit;
        # immediate re-entry on the same pair is how fees compound
        self._reentry: dict[str, float] = {}
        # account-level drawdown brake (10% off peak pauses new entries,
        # 7% releases); peak is seeded from recent equity history at start
        self._peak_equity = 0.0
        self._dd_braked = False
    ...
    def start(self) -> tuple[bool, str]:
        with self._lock:
            ...
            try:
                ...
                if mode == "paper":
                    ...
                    self.broker = PaperBroker(float(cfg.get("paper_balance", 10000)),
                                              taker_fee=fee_side)
                    # demo fills also cross the venue's real half-spread
                    # (measured ~0.55% median on ANDX alts) — the demo must
                    # not flatter what live execution would cost
                    self.broker.spread_fn = (
                        lambda s: self.market.spread_pct(s, margin=not self.spot_only))
                else:
            ...
            # drawdown brake: seed the equity peak from recent history so a
            # restart can't wipe the account's memory of where it stood
            self._dd_braked = False
            try:
                rows = store.equity_curve(mode)
                self._peak_equity = max([r.get("equity", 0.0) for r in rows] + [0.0])
            except Exception:
                self._peak_equity = 0.0
    ...
    # -------------------------------------------------------- limit orders
    # Working orders, checked against fresh prices every tick. A fill goes
    # through manual_open, so every rail (caps, kill switch, watched
    # universe) applies at FILL time, not just placement time.

    def place_limit_order(self, symbol: str, side: int, usd: float,
                          limit_price: float, stop=None, take_profit=None,
                          expires_h=None) -> dict:
        if not self.running or not self.broker:
            return {"error": "the engine isn't running"}
        if self.broker.mode != "paper":
            return {"refused": "limit orders run on the demo account for now"}
        if side not in (1, -1):
            return {"error": "side must be long or short"}
        watched = list(self.strategies.keys()) if self.strategies else []
        if symbol not in watched:
            return {"error": "I only work orders on coins I watch: "
                             + (", ".join(watched) or "none yet")}
        px = self.prices.get(symbol)
        if not px:
            return {"error": f"no live price for {symbol} yet"}
        try:
            usd, limit_price = float(usd), float(limit_price)
        except (TypeError, ValueError):
            return {"error": "size and limit must be numbers"}
        if usd < 10:
            return {"error": "minimum order is $10"}
        if side == 1 and limit_price >= px:
            return {"error": f"{symbol} is at {px:.6g} — a buy limit at "
                             f"{limit_price:.6g} fills instantly. Want a "
                             "market buy instead?"}
        if side == -1 and limit_price <= px:
            return {"error": f"{symbol} is at {px:.6g} — a sell limit at "
                             f"{limit_price:.6g} fills instantly. Want a "
                             "market short instead?"}
        bad = self._validate_exits(side, limit_price, stop, take_profit)
        if bad:
            return {"error": bad + " (checked against your limit price)"}
        if len(store.list_pending_orders(self.broker.mode)) >= 10:
            return {"error": "10 working orders is my cap — cancel one first"}
        expires_ts = (time.time() + float(expires_h) * 3600) if expires_h else None
        oid = store.add_pending_order(
            symbol, "long" if side == 1 else "short", usd, limit_price,
            stop, take_profit, expires_ts, self.broker.mode)
        side_txt = "BUY" if side == 1 else "SHORT"
        self.log(f"LIMIT ORDER #{oid}: {side_txt} ${usd:,.0f} {symbol} @ "
                 f"{limit_price:.6g}" + (f", expires in {expires_h}h"
                                         if expires_h else " (GTC)"))
        return {"order_placed": {"id": oid, "symbol": symbol,
                                 "side": "long" if side == 1 else "short",
                                 "usd": usd, "limit_price": limit_price,
                                 "current_price": px,
                                 "good_til": "cancelled" if not expires_h
                                             else f"{expires_h}h"},
                "note": "checked every tick; fills at the touch through the "
                        "same rails as any trade"}

    def cancel_pending_order(self, order_id) -> dict:
        try:
            ok = store.resolve_pending_order(int(order_id), "cancelled",
                                             "cancelled by user")
        except (TypeError, ValueError):
            return {"error": "bad order id"}
        if ok:
            self.log(f"limit order #{order_id} cancelled")
        return {"cancelled": bool(ok)} if ok else \
            {"error": "no open order with that id"}

    def _check_pending_orders(self):
        if not self.broker or self.broker.mode != "paper":
            return
        now = time.time()
        for o in store.list_pending_orders(self.broker.mode):
            if o["expires_ts"] and o["expires_ts"] <= now:
                store.resolve_pending_order(o["id"], "expired",
                                            "time limit reached")
                self.log(f"limit order #{o['id']} ({o['symbol']}) expired")
                self.notify("ANDX AI — order expired",
                            f"Your {o['symbol'].split('/')[0]} limit order "
                            "expired unfilled.")
                continue
            ...
            touched = (px <= o["limit_price"]) if side == 1 \
                else (px >= o["limit_price"])
            if not touched:
                continue
            result = self.manual_open(o["symbol"], side, o["usd"],
                                      o["stop"], o["take_profit"])
            if "opened" in result:
                entry = result["opened"].get("entry")
                store.resolve_pending_order(o["id"], "filled",
                                            f"filled @ {entry:.6g}")
                self.log(f"LIMIT FILLED #{o['id']}: {o['side']} "
                         f"{o['symbol']} @ {entry:.6g} (limit "
                         f"{o['limit_price']:.6g})")
                self.notify("ANDX AI — limit order filled",
                            f"{o['symbol'].split('/')[0]} touched "
                            f"{o['limit_price']:.6g} — you're in @ "
                            f"{entry:.6g}.")
            else:
                why = result.get("error") or result.get("refused") or "failed"
                store.resolve_pending_order(o["id"], "cancelled", why)
                self.log(f"limit order #{o['id']} touched but couldn't "
                         f"fill: {why}", "warn")
                self.notify("ANDX AI — order not filled",
                            f"{o['symbol'].split('/')[0]} hit your level "
                            f"but the order was refused: {why}")

    def _orders_snapshot(self) -> list[dict]:
        try:
            mode = self.broker.mode if self.broker else "paper"
            return store.list_pending_orders(mode)
        except Exception:
            return []
    ...
    def _tick(self, cfg: dict, risk_cfg: risk_mod.RiskConfig, gen: int | None = None):
        ...
        # -- working limit orders: fill the ones the market just touched
        try:
            self._check_pending_orders()
        except Exception as e:
            self.log(f"limit-order check failed: {e}", "warn")
        ...
        # -- drawdown brake: in a >10% hole from the ROLLING 30-day equity
        # peak, stop opening new positions until the account recovers to
        # within 7%. Validated to cut max drawdown AND slightly improve
        # return — pressing while losing is how drawdowns become disasters.
        # The peak is a rolling window, never all-time: an all-time peak is
        # an absorbing state (a flat, braked account can never climb back to
        # it, freezing the bot forever — caught by the year-long backtest).
        # Exits, stops and take-profits keep running untouched.
        try:
            self._peak_equity = max(store.peak_equity(self.broker.mode, 30), equity)
        except Exception:
            self._peak_equity = max(getattr(self, "_peak_equity", 0.0) or equity, equity)
        if not self._dd_braked and equity < 0.90 * self._peak_equity:
            self._dd_braked = True
            self.log(f"drawdown brake ON — equity {equity:,.0f} is "
                     f"{100 * (1 - equity / self._peak_equity):.1f}% off the "
                     f"peak; no new entries until recovery", "warn")
            self.notify("ANDX AI — drawdown brake",
                        "I'm in a drawdown, so I've stopped opening new "
                        "positions until the account recovers. Exits stay on.")
        elif self._dd_braked and equity > 0.93 * self._peak_equity:
            self._dd_braked = False
            self.log("drawdown brake OFF — account recovered, trading resumes")
            self.notify("ANDX AI — brake released",
                        "Account recovered — I'm allowed to open positions again.")
        ...
        for symbol in cfg["symbols"]:
                ...
                if pos.stop and risk_mod.stop_hit(pos.side, price, pos.stop):
                    ...
                    if trade:
                        ...
                        self.notify("ANDX AI — stop hit",
                                    ...
                                    f"{trade['pnl']:+,.2f}")
                        self._persist_positions(cfg)
                        self._reentry[symbol] = time.time() + 2 * TIMEFRAME_SECONDS.get(cfg["timeframe"], 3600)
                        self._contra.pop(symbol, None)
                    pos = None
            ...
            if pos and desired == pos.side:
                self._contra.pop(symbol, None)  # signal agrees again
                # real round-trip cost from config, not a hardcoded guess —
                # the break-even layer sits exactly this far past entry
                fee_rt = 2 * float(cfg.get("fee_pct_per_side",
                                           1.0 if self.spot_only else 0.25)) / 100.0 + 0.001
            ...
            if pos:
                # Trend positions: one bar's wobble is noise, not an exit.
                # The contra reading must hold on two DIFFERENT closed
                # candles, and no signal exit inside the first full bar —
                # stops and take-profits above still fire instantly.
                # Mean-reversion exits (mid-band touch) stay immediate:
                # that touch IS the profit-take.
                if strategy.trailing:
                    bar_ts = float(closed["ts"].iloc[-1])
                    cnt, last_ts = self._contra.get(symbol, (0, 0.0))
                    if bar_ts != last_ts:
                        cnt += 1
                        self._contra[symbol] = (cnt, bar_ts)
                    tf_secs = TIMEFRAME_SECONDS.get(cfg["timeframe"], 3600)
                    if cnt < 2 or time.time() - pos.opened_at < tf_secs:
                        self.last_signals[symbol] = "exit signal — awaiting confirmation"
                        continue
                try:
                    with self._order_lock:
                        trade = self.broker.close(symbol, price)
            ...
            if desired != 0 and not self.kill_switch_tripped:
                ...
                equity = self.broker.equity(self.prices) + self._guard_value()
                notional = self.broker.open_notional(self.prices) + self._guard_value()
                # Weak-regime haircut: entries taken with ADX under 22 are
                # the lowest-quality cohort — take them at reduced size
                # rather than full risk.
                sig_adx = float(adx(closed, 14).iloc[-1])
                regime_mult = 0.6 if sig_adx < 22 else 1.0
                qty, reason = risk_mod.position_size(
                    equity, price, atr_val, risk_cfg, notional,
                    risk_mult=self._risk_multiplier(symbol) * regime_mult)
    ...
    def _entry_gate(self, symbol: str, desired: int, price: float,
        ...
        # 0) account-level drawdown brake
        if self._dd_braked:
            return "drawdown brake on — waiting for the account to recover"
        # 0.5) macro regime: when BTC is under its 200-day average the whole
        #      crypto market is bear — alt longs there cut a losing year's
        #      damage by 70% in the validation, simply by not being taken
        if desired == 1 and not self._btc_ok():
            return "BTC below its 200-day — bear market, standing down"
        # 1) universal re-entry cooldown: after ANY exit on a pair, sit out
        #    two full bars — instant re-entry is how fees compound
        if time.time() < self._reentry.get(symbol, 0):
            return "just exited — cooling down"
        # 2) per-symbol stop-out cooldown: two losing stop-outs today means
        #    this pair is churning fees, not trending
        if self._governor.get("stopouts", {}).get(symbol, 0) >= 2:
            return "2 stop-outs today — cooling off"
        # 3) proven-loser veto: enough recorded trades, negative total, low
        #    win rate — stop feeding it (the risk governor already half-sizes
        #    milder cases)
        stats = self._perf.get(symbol)
        if stats and stats["n"] >= 6 and stats["total"] < 0 and stats["win_rate"] < 0.30:
            return "poor recorded expectancy on this pair"
        # 4) portfolio risk budget: total open risk-to-stops capped, because
        #    ten correlated crypto longs are one trade wearing ten hats
        equity = self.broker.equity(self.prices) + self._guard_value()
        if equity > 0:
            # ENGINE-managed positions only: the user's own manual trades and
            # Sphinx portfolio legs are their allocation decisions, not the
            # engine's risk budget — counting them starves the engine outright
            # (observed live: two portfolio legs = 15.4%, every entry blocked)
            heat = sum(
                p.qty * abs(self.prices.get(s, p.entry) - p.stop)
                for s, p in self.broker.positions.items()
                if p.stop and getattr(p, "strategy", "") not in ("manual", "portfolio")
            ) / equity
            if heat >= 0.04:
                return f"portfolio risk budget full ({heat * 100:.1f}% at stops)"
        # 5) multi-timeframe confirmation: entries need the higher timeframe
        #    BEHIND them, not merely not-against. Out-of-sample this cut
        #    flat-market losses nearly in half and halved drawdown. A
        #    missing HTF read blocks entries (fail-closed); exits are
        #    never gated.
        bias, bias_adx = self._htf(symbol, cfg)
        if bias != desired:
            return "no higher-timeframe trend behind it"
        # 6) execution-venue spread
        spread = self.market.spread_pct(symbol, margin=not self.spot_only)
        if spread > 0.015:
            return f"spread {spread * 100:.1f}%"
        # 7) fee-aware minimum edge: the initial stop distance must clear the
        #    round-trip cost with REAL room, or the trade is a fee donation.
        #    3x, not 1.5x: the recorded gross edge per trade is well under
        #    2 round trips, so marginal signals lose money net of costs.
        fee_side = float(cfg.get("fee_pct_per_side",
                                 1.0 if self.spot_only else 0.25)) / 100.0
        round_trip = 2 * fee_side + spread
        if risk_cfg.atr_stop_mult * atr_val < 3.0 * round_trip * price:
            return "expected move below round-trip fees"
        return None

    def _btc_ok(self) -> bool:
        """True while BTC's last closed day is above its 200-day SMA.
        Cached an hour; fails OPEN — the macro overlay must never freeze
        the bot on a data hiccup (per-symbol gates still apply)."""
        cached = getattr(self, "_btc_regime", None)
        if cached and time.time() - cached[0] < 3600:
            return cached[1]
        ok = True
        try:
            raw = self.market.ohlcv("BTC/USDT", "1d", limit=220)
            if len(raw) >= 201:
                closes = [r[4] for r in raw[:-1]]  # closed days only
                sma = sum(closes[-200:]) / 200.0
                ok = closes[-1] > sma
        except Exception:
            ok = True
        self._btc_regime = (time.time(), ok)
        return ok
    ...
    def snapshot(self) -> dict:
        ...
        return {
            ...
            "open_orders": self._orders_snapshot(),
            "error": self.error,
            "kill_switch": self.kill_switch_tripped,
            "dd_brake": self._dd_braked,
