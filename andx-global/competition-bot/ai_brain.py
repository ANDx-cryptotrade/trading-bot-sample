...
- Limit orders too: "buy X if it drops to Y" becomes a working order \
checked every tick, good-til-cancelled unless they set a time limit. \
Confirm the level vs the current price, list or cancel on request, and \
when one fills they get a notification with the real entry.
- Fees: get_fee_summary shows the rate and what their trading has \
actually cost, 30-day and all-time. Volunteer it when they grumble about \
costs — the receipt beats reassurance.
- Backtest record: get_backtest_record is a SIMULATED replay of the \
strategy over real historical prices with honest fees. Present it as a \
backtest every time, never as live results, and never mix its numbers \
into the live account's stats. Simulated and live are different words \
for a reason.
...
def t_place_limit_order(args):
    ...
    try:
        return ENGINE.place_limit_order(
            _norm_symbol(args.get("symbol")), side,
            float(args.get("usd_amount")), float(args.get("limit_price")),
            args.get("stop_price"), args.get("take_profit_price"),
            args.get("expires_hours"))
    except (TypeError, ValueError):
        return {"error": "usd_amount and limit_price must be numbers"}


def t_list_open_orders(_args):
    mode = ENGINE.broker.mode if ENGINE.broker else "paper"
    orders = store.list_pending_orders(mode)
    for o in orders:
        o["current_price"] = ENGINE.prices.get(o["symbol"])
    return {"open_orders": orders,
            "recently_resolved": store.list_pending_orders(mode, status="",
                                                           limit=8)}


def t_cancel_order(args):
    guard = _paper_only()
    if guard:
        return guard
    return ENGINE.cancel_pending_order(args.get("id"))


def t_get_fee_summary(_args):
    mode = ENGINE.broker.mode if ENGINE.broker else "paper"
    cfg = store.load_config()
    rate = float(cfg.get("fee_pct_per_side", 0.25)) / 100
    vol_all = store.cumulative_volume(mode)
    vol_30d = store.cumulative_volume(mode, time.time() - 30 * 86400)
    return {"fee_rate_per_side_pct": rate * 100,
            "traded_volume_30d": round(vol_30d, 2),
            "fees_paid_30d_est": round(vol_30d * rate, 2),
            "traded_volume_all_time": round(vol_all, 2),
            "fees_paid_all_time_est": round(vol_all * rate, 2),
            "note": "estimated from journaled fills at the current fee "
                    "setting; every quoted P&L already includes fees"}


def t_get_backtest_record(_args):
    rec = store.backtest_record()
    rec["note"] = ("SIMULATED strategy replay over real historical prices "
                   "with honest fees — always present it as a backtest, "
                   "never as live trading results, and never blend it "
                   "with the live account's stats")
    return rec
...
TOOLS = [
    ...
    {"name": "place_limit_order",
     "description": "Work a limit order: buy/short only when the price reaches the user's level ('buy $500 of BTC if it drops to 75,000'). Checked every tick; fills through the same rails as any trade; good-til-cancelled unless expires_hours is set. Only on the user's clear request with a size and level.",
     "input_schema": {"type": "object", "required": ["symbol", "side", "usd_amount", "limit_price"],
                      "properties": {
                          "symbol": {"type": "string"},
                          "side": {"type": "string", "enum": ["long", "short"]},
                          "usd_amount": {"type": "number"},
                          "limit_price": {"type": "number"},
                          "stop_price": {"type": "number", "description": "optional, validated against the limit price"},
                          "take_profit_price": {"type": "number"},
                          "expires_hours": {"type": "number", "description": "optional; omit for good-til-cancelled"}}}},
    {"name": "list_open_orders",
     "description": "Working limit orders with current prices, plus recently filled/cancelled/expired ones.",
     "input_schema": {"type": "object", "properties": {}}},
    {"name": "cancel_order",
     "description": "Cancel one working limit order by id, when the user asks.",
     "input_schema": {"type": "object", "required": ["id"],
                      "properties": {"id": {"type": "integer"}}}},
    {"name": "get_fee_summary",
     "description": "What trading has cost: fee rate, 30-day and all-time traded volume and estimated fees paid. Use for 'how much have I paid in fees', 'what are the fees'.",
     "input_schema": {"type": "object", "properties": {}}},
    {"name": "get_backtest_record",
     "description": "The strategy's SIMULATED backtest record over real historical prices: trade count, win rate, net P&L, recent simulated trades. Use when asked about the backtest, the strategy's historical record, or 'how has the strategy done historically'. Always call it a simulation.",
     "input_schema": {"type": "object", "properties": {}}},
]
...
_TOOL_IMPL = {
    ...
    "place_limit_order": t_place_limit_order,
    "list_open_orders": t_list_open_orders,
    "cancel_order": t_cancel_order,
    "get_fee_summary": t_get_fee_summary,
    "get_backtest_record": t_get_backtest_record,
}
