...
# ANDX_BOT_DATA points an instance at its own data directory (per-user
# isolation when the gateway runs many bots); default is the code directory,
# which keeps the single-user desktop setup exactly as before.
BASE_DIR = os.environ.get("ANDX_BOT_DATA") or os.path.dirname(os.path.abspath(__file__))
...
DEFAULT_CONFIG = {
    ...
    "timeframe": "15m",   # validated home of the strategy (see backtest.py)
    "strategy": "auto",
...
def _db():
    ...
    conn.execute(
        """CREATE TABLE IF NOT EXISTS pending_orders (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            symbol TEXT, side TEXT, usd REAL, limit_price REAL,
            stop REAL, take_profit REAL, status TEXT,
            created_ts REAL, resolved_ts REAL, expires_ts REAL,
...
# -------------------------------------------------------- limit orders
# Working orders the engine checks against fresh prices every tick.
# GTC unless expires_ts is set; fills become normal (manual) positions.

def add_pending_order(symbol: str, side: str, usd: float, limit_price: float,
                      stop, take_profit, expires_ts, mode: str) -> int:
    with _lock, _db() as conn:
        cur = conn.execute(
            "INSERT INTO pending_orders (symbol, side, usd, limit_price, "
            "stop, take_profit, status, created_ts, resolved_ts, expires_ts, "
            "result, mode) VALUES (?,?,?,?,?,?, 'open', ?, NULL, ?, NULL, ?)",
            (symbol, side, float(usd), float(limit_price),
             float(stop) if stop else None,
             float(take_profit) if take_profit else None,
             time.time(), float(expires_ts) if expires_ts else None, mode))
        return cur.lastrowid


def list_pending_orders(mode: str, status: str = "open",
                        limit: int = 30) -> list[dict]:
    q = ("SELECT id, symbol, side, usd, limit_price, stop, take_profit, "
         "status, created_ts, expires_ts, result FROM pending_orders "
         "WHERE mode=? ")
    args = [mode]
    if status:
        q += "AND status=? "
    ...
    cols = ["id", "symbol", "side", "usd", "limit_price", "stop",
            "take_profit", "status", "created_ts", "expires_ts", "result"]
    return [dict(zip(cols, r)) for r in rows]


def resolve_pending_order(order_id: int, status: str, result: str = "") -> bool:
    with _lock, _db() as conn:
        cur = conn.execute(
            "UPDATE pending_orders SET status=?, resolved_ts=?, result=? "
            "WHERE id=? AND status='open'",
            (status, time.time(), str(result)[:200], int(order_id)))
        return cur.rowcount > 0
...
def recent_trades(limit: int = 50) -> list[dict]:
    # mode='backtest' rows are the imported simulated record — they belong
    # in the labeled Backtested Record panel, never in the live trade feed
    with _lock, _db() as conn:
        rows = conn.execute(
            "SELECT ts, symbol, side, qty, entry, exit_price, pnl, strategy, mode, reason "
            "FROM trades WHERE mode != 'backtest' ORDER BY id DESC LIMIT ?", (limit,)
        ).fetchall()
    cols = ["ts", "symbol", "side", "qty", "entry", "exit", "pnl", "strategy", "mode", "reason"]
    return [dict(zip(cols, r)) for r in rows]


def peak_equity(mode: str, days: int = 30) -> float:
    """Highest recorded equity in the last N days — the drawdown brake's
    reference. Rolling on purpose: an all-time peak is an absorbing state
    (a braked account can never recover to a level it can no longer reach)."""
    with _lock, _db() as conn:
        row = conn.execute(
            "SELECT MAX(equity) FROM equity WHERE mode=? AND ts>=?",
            (mode, time.time() - days * 86400)).fetchone()
    return float(row[0] or 0.0)


def backtest_record(limit: int = 25) -> dict:
    """The imported simulated record (mode='backtest'): a labeled backtest
    over real historical prices, kept strictly out of the live feeds.
    Everything returned here must be presented as SIMULATED results."""
    with _lock, _db() as conn:
        agg = conn.execute(
            "SELECT COUNT(*), SUM(CASE WHEN pnl>0 THEN 1 ELSE 0 END), "
            "SUM(pnl), MIN(ts), MAX(ts) FROM trades WHERE mode='backtest'"
        ).fetchone()
        rows = conn.execute(
            "SELECT ts, symbol, side, qty, entry, exit_price, pnl, reason "
            "FROM trades WHERE mode='backtest' ORDER BY ts DESC LIMIT ?", (limit,)
        ).fetchall()
    n, wins, pnl, t0, t1 = agg
    cols = ["ts", "symbol", "side", "qty", "entry", "exit", "pnl", "reason"]
    return {
        "simulated": True,
        "count": n or 0,
        "wins": wins or 0,
