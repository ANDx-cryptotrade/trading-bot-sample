...
class RiskConfig:
    ...
    atr_stop_mult: float = 2.5         # INITIAL stop distance in ATRs (sizing anchor)
    atr_trail_mult: float = 5.0        # TRAILING distance in ATRs — deliberately wide:
                                       # trend exits belong to the signal; the trail is
                                       # disaster insurance, and trailing at the tight
                                       # initial distance was measured to eject the bot
                                       # from most winning trends on normal retraces
...
def manage_stop(side: int, entry: float, stop: float, r_value: float,
    ...
    if trailing:
        candidates.append(close - side * cfg.atr_trail_mult * atr_value)
    profit_r = ((close - entry) * side / r_value) if r_value and r_value > 0 else 0.0
    if profit_r >= 1.0:
        be = entry * (1 + side * fee_roundtrip)
        # Only arm break-even once price has cleared it with real room
        # (0.25R). A BE level at or past the market is not protection —
        # it is an instant stop-out that amputates every winner whose 1R
        # is smaller than the fee buffer.
        if (close - be) * side > 0.25 * r_value:
            candidates.append(be)
    if profit_r >= 1.5 and trailing:
        candidates.append(hw - side * cfg.atr_trail_mult * atr_value)
    if not candidates:
        return stop, hw
